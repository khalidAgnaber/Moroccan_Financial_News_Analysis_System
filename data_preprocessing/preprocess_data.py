"""
Script to preprocess the collected financial news data using word embeddings
"""
...
def main():
    ...
    try:
        ...
        # Check if tokens are already lists or if they need to be evaluated
        tokens_list = []
            ...
            if isinstance(token_data, list):
                ...
                try:
                    ...
                    if isinstance(tokens, list):
                        ...
                        # If not a list, create a list with the single token
                        tokens_list.append([token_data])
                except (ValueError, SyntaxError):
                    # If evaluation fails, split by spaces as fallback
                    tokens_list.append(token_data.split())
            else:
                # For any other type, convert to string and add as a single token
                tokens_list.append([str(token_data)])
                
        logger.info(f"Extracted {len(tokens_list)} token lists")
        
        # Print a sample of the tokens to debug
        if tokens_list:
            sample_idx = min(5, len(tokens_list) - 1)
            logger.info(f"Sample tokens (document {sample_idx}): {tokens_list[sample_idx][:10]}")
        
        # Train phraser for multi-word expressions
        ...
        # Visualize embeddings if we have category data
