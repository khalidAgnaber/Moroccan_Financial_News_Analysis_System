...
def get_data_by_company(df, company_name):
    """Filter data for a specific company."""
    if 'companies' not in df.columns:
        logger.error("No 'companies' column in the data. Run preprocessing first.")
        return None
    
    # Filter articles mentioning the company
    company_df = df[df['companies'].str.contains(company_name, na=False)]
...
def get_data_statistics(df):
    ...
    # Companies coverage (if preprocessed)
    if 'companies' in df.columns:
        # Count unique companies
        all_companies = []
        for companies_str in df['companies'].dropna():
            companies = companies_str.split(',')
            all_companies.extend(companies)
        
        stats['unique_companies'] = len(set(all_companies))
...
def backup_raw_data():
    """Create a backup of the raw data file."""
    file_path = RAW_DATA_DIR / "moroccan_financial_news.csv"
